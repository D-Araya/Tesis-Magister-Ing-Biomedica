...
"""IMPORTANTE, EL ANGULO DE YAW SE DEBE CANCELAR DEBIDO A DESEAR LOGRAR EL AJUSTE...
...
datas = np.loadtxt(r'C:\Users\Casa\PycharmProjects\Leap with Python\signal\EstudioDeCaso\9.csv', delimiter=',', unpack=True)
...
for j in range(len(pitch)):
    ...
    est1[j] = np.dot(matriz1[j], est[j])   # O tambien est1[j] = np.dot(rot1[:], mat.transpose()[j])
#print est1
...
for j in range(len(roll)):
    ...
    est2[j] = np.dot(matriz2[j], est1[j])   # O tambien est2[j] = np.dot(rot2[:], mat.transpose()[j])
#print est2
